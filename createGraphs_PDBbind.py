...
fullSetPath = "PDBbind_all" # path folder containing complexes for refined set, general set and core set


outDir = "interaction_graphs"


aminoAcids = ['ALA', 'VAL', 'ASP', 'LYS', 'ASN', 'LEU', 'ILE', 'CYS', 'THR', 'PHE',
              'GLY', 'TRP', 'ARG', 'TYR', 'GLN', 'PRO', 'HIS', 'MET', 'GLU', 'SER',
              ' MG', ' ZN', ' MN', 'LLP', 'KCX', ' CO', ' CA', ' NA', 'MSE'] 
...
types = [rdkit.Chem.rdchem.HybridizationType.S,
...
def createGraphs(index):
    for pdbID in index:
        ...
        if ligand is None:
            ...
            with open(f"{outDir}/{pdbID}_graph.pt","wb") as file:
                torch.save(graph_pt,file)

split = np.array_split(index,3)
with Pool(3) as p:
    p.map(createGraphs,split)
